...
本文件是 YeMind 的项目级开发入口。所有后续开发以 **v0.8.0** 为当前稳定基线，并结合 `.agents/skills/superpowers-*` 中的 Superpowers skills 执行。

## 1. Baseline / 当前基线

- 工程包名：`siyuan-yemind`
- 永久兼容插件 ID：`siyuan-yemind-zen`
- 显示名称：`YeMind`
- 当前开发基线：`v0.8.0`
- 不得回退或破坏 v0.5.10 至 v0.8.0 已完成的稳定性修复与交互契约。
...
- 输出包名称必须包含明确版本和用途，例如：`siyuan-yemind-v0.8.0-dev.zip`。
