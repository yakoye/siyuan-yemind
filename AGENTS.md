...
本文件是 YeMind Zen 的项目级开发入口。所有后续开发以 **v0.5.12** 为当前稳定基线，并结合 `.agents/skills/superpowers-*` 中的 Superpowers skills 执行。
...
- 显示名称：`YeMind Zen`
- 当前开发基线：`v0.5.12`
- 不得回退或破坏 v0.5.10 至 v0.5.12 已完成的稳定性与交互修复。
...
5. 拖动结构继续由上游 `MOVE_NODE_TO`、`INSERT_BEFORE`、`INSERT_AFTER` 决定；项目代码只能增强提示，不另建第二套结构拖拽算法。
6. 插件 bootstrap 必须先于可选宿主界面注册启动；单个注册异常不得让正式仓库保持未加载。
7. 大纲文字和结构编辑必须通过上游节点命令完成，不直接改写 `MindMapTree.children`。
8. 大纲必须保留提交前置、结构事务串行化、IME 保护和 UID/光标恢复；不得退回每次按键后无条件重建并丢失焦点的实现。
9. 拖拽候选必须保留逐帧合并与稳定目标机制；最终落点仍只能由上游 Drag 的结构命令决定。
...
- 输出包名称必须包含明确版本和用途，例如：`siyuan-yemind-zen-v0.5.12-dev.zip`。
