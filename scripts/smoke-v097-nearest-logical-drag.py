...
with sync_playwright() as p:
    ...
    page.evaluate("""async()=>{
      ...
      const map=await plugin.repository.create('v0.9.7 Nearest Logical Drag Smoke','logicalStructure');
    ...
    if not neutral_outline['ghost'] or neutral_outline['drops'] == 0:
        raise RuntimeError(f'Outline full-row target missing: preview={neutral_outline}')
    ...
    # ----- Canvas nearest-node drag -----
    page.click('[data-action="view-map"]')
    page.wait_for_timeout(250)

    # The green parent line exists from the first drag frame. In open space it
    # falls back to the original parent rather than disappearing.
    c = canvas_box(page, 'Node C')
    page.mouse.move(c['x'] + 20, c['y'] + c['height'] / 2); page.mouse.down()
    page.mouse.move(c['x'] + 330, c['y'] + 260, steps=14); page.wait_for_timeout(80)
    fallback_guides = visible_guides(page)
    if not any(item['dash'] == '6 6' for item in fallback_guides):
        raise RuntimeError(f'Continuous original-parent guide missing: {fallback_guides}')

    # Moving into A's left-upper local quadrant chooses BEFORE under Root. The
    # line remains visible and room preview follows the same candidate.
    a = canvas_box(page, 'Node A'); b_before = canvas_box(page, 'Node B')
    page.mouse.move(a['x'] + 8, a['y'] + 3, steps=12); page.wait_for_timeout(70)
    before_guides = visible_guides(page)
    before_path = next((item['d'] for item in before_guides if item['dash'] == '6 6'), '')
    shifted_a = canvas_box(page, 'Node A'); shifted_b = canvas_box(page, 'Node B')
    if not before_path or shifted_a['y'] <= a['y'] + 8 or shifted_b['y'] <= b_before['y'] + 8:
        raise RuntimeError(f'Nearest BEFORE preview failed: guides={before_guides}, A={shifted_a}, B={shifted_b}')

    # Moving across the local cross into A's right side switches the candidate
    # parent to A in the same pointer frame. The same dashed element stays
    # visible but its path changes to the new parent.
    page.mouse.move(a['x'] + a['width'] + 30, a['y'] + a['height'] / 2, steps=10); page.wait_for_timeout(70)
    child_guides = visible_guides(page)
    child_path = next((item['d'] for item in child_guides if item['dash'] == '6 6'), '')
    if not child_path or child_path == before_path:
        raise RuntimeError(f'Real-time parent guide did not switch to A: before={before_path}, child={child_path}')

    # Returning to B's sibling side changes the guide back to Root without ever
    # hiding it, then releasing performs the corresponding sibling move.
    b_box = canvas_box(page, 'Node B')
    page.mouse.move(b_box['x'] + 8, b_box['y'] + 3, steps=10); page.wait_for_timeout(70)
    sibling_guides = visible_guides(page)
    sibling_path = next((item['d'] for item in sibling_guides if item['dash'] == '6 6'), '')
    if not sibling_path or sibling_path == child_path:
        raise RuntimeError(f'Real-time parent guide did not switch back to Root: child={child_path}, sibling={sibling_path}')
    if any(item['stroke'] == '#176b50' and not item['dash'] for item in sibling_guides):
        raise RuntimeError(f'Logical canvas insertion line returned: {sibling_guides}')
    page.mouse.up()
    ...
    page.click('[data-action="undo"]'); wait_shape(page, initial_compact); page.wait_for_timeout(180)

    # A direct CHILD drop uses A as the parent, keeps metadata and is one-step
    # undoable.
    c = canvas_box(page, 'Node C'); a = canvas_box(page, 'Node A')
    page.mouse.move(c['x'] + 20, c['y'] + c['height'] / 2); page.mouse.down()
    page.mouse.move(a['x'] + a['width'] + 30, a['y'] + a['height'] / 2, steps=14); page.wait_for_timeout(80)
    direct_child_guides = visible_guides(page)
    if not any(item['dash'] == '6 6' for item in direct_child_guides):
        raise RuntimeError(f'Direct CHILD parent guide missing: {direct_child_guides}')
    page.mouse.up()
    page.wait_for_function("""()=>{const a=window.__smoke.plugin.repository.get(window.__smoke.map.id).data.children.find(n=>n.data.uid==='a');return a.children.some(n=>n.data.uid==='c')}""", timeout=8000)
    moved_canvas = tree_shape(page)
    moved_c = next(child for child in next(node for node in moved_canvas['children'] if node['uid'] == 'a')['children'] if child['uid'] == 'c')
    if moved_c['tag'] != ['keep-c'] or moved_c['note'] != 'keep-c-note':
        raise RuntimeError(f'Canvas nearest drag lost metadata: {moved_c}')
    page.click('[data-action="undo"]'); wait_shape(page, initial_compact); page.wait_for_timeout(180)

    # Escape cancels while the continuous green line is visible.
    c = canvas_box(page, 'Node C'); a = canvas_box(page, 'Node A')
    page.mouse.move(c['x'] + 20, c['y'] + c['height'] / 2); page.mouse.down()
    page.mouse.move(a['x'] + a['width'] + 24, a['y'] + a['height'] / 2, steps=10); page.wait_for_timeout(60)
    escape_guides = visible_guides(page)
    page.keyboard.press('Escape'); page.mouse.up(); page.wait_for_timeout(220)
    if not any(item['dash'] == '6 6' for item in escape_guides) or compact(tree_shape(page)) != initial_compact:
        raise RuntimeError(f'Canvas continuous-guide Escape failed: guides={escape_guides}, tree={tree_shape(page)}')
    ...
    print({
        ...
        'canvasFallbackGuides': fallback_guides,
        'canvasBeforeGuides': before_guides,
        'canvasChildGuides': child_guides,
        'canvasSiblingGuides': sibling_guides,
