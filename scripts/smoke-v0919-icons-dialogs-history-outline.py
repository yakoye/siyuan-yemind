...
with sync_playwright() as p:
    ...
    marker=page.evaluate("""()=>{const option=document.querySelector('.ymz-marker-option');const grid=document.querySelector('.ymz-marker-grid');return{tabs:document.querySelectorAll('.ymz-marker-dialog .ymz-asset-tab').length,sections:document.querySelectorAll('.ymz-marker-group').length,options:document.querySelectorAll('.ymz-marker-option').length,width:window.__lastDialog.options.width,height:window.__lastDialog.options.height,customClose:window.__lastDialog.options.hideCloseIcon===true&&!!document.querySelector('.ymz-marker-dialog .ymz-local-asset-dialog__close'),footer:getComputedStyle(document.querySelector('.ymz-marker-dialog .ymz-local-asset-dialog__footer')).justifyContent,optionBackground:getComputedStyle(option).backgroundColor,gridBackground:getComputedStyle(grid).backgroundColor}}""")
    if marker['tabs']!=9 or marker['sections']!=8 or marker['options']!=126 or marker['width']!='600px' or marker['height']!='620px' or not marker['customClose'] or marker['footer']!='flex-end':
    ...
    clipart=page.evaluate("""()=>{const grid=document.querySelector('.ymz-clipart-grid');const option=document.querySelector('.ymz-clipart-option');return{items:document.querySelectorAll('.ymz-clipart-option').length,more:!!document.querySelector('[data-action=clipart-more]'),width:window.__lastDialog.options.width,height:window.__lastDialog.options.height,customClose:window.__lastDialog.options.hideCloseIcon===true&&!!document.querySelector('.ymz-clipart-dialog .ymz-local-asset-dialog__close'),gridBackground:getComputedStyle(grid).backgroundColor,optionBackground:getComputedStyle(option).backgroundColor}}""")
    if clipart['items']!=764 or clipart['more'] or clipart['width']!='660px' or clipart['height']!='620px' or not clipart['customClose']:
