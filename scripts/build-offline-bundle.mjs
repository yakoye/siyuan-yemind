...
const walk = (directory) => {
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) walk(full);
      ...
      sources.set(canonical(relative), entry.name.endsWith('.json') ? `export default ${content.trim()};\n` : content);
    }
  }
};
...
const banner = `"use strict";\n// YeMind v0.9.21 offline release bundle. Generated from current source and the v0.9.0 verified dependency Source Map.\n`;
...
const manifestPath = path.resolve(root, process.env.YEMIND_BUNDLE_MANIFEST ?? 'docs/offline-bundle-manifest-v0.9.21.json');
