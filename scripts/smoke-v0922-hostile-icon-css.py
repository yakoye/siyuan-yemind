...
with sync_playwright() as p:
    ...
    result=page.evaluate("""()=>{
      const toolbarSelectors=['[data-action=open-search] .ymz-icon-slot','[data-action=project-style] .ymz-icon-slot','[data-action=undo] .ymz-icon-slot','[data-action=redo] .ymz-icon-slot'];
      const toolbar=toolbarSelectors.map(selector=>document.querySelector(selector)).filter(Boolean);
      const menu=[...document.querySelectorAll('.b3-menu .ymz-icon-slot--source')];
      const slots=[...toolbar,...menu];
      const all=slots.flatMap(slot=>[...slot.querySelectorAll('img.ymz-operation-icon')]);
      ...
      return {toolbar:toolbar.length,menu:menu.length,slots:slots.length,all:all.length,loaded:all.every(img=>img.complete&&img.naturalWidth>0&&img.naturalHeight>0),dataUris:all.every(img=>img.src.startsWith('data:image/svg+xml;base64,')),inlineChildren:all.reduce((n,img)=>n+img.childElementCount,0),inlineSuppliedSvg,tags:[...new Set(all.map(img=>img.tagName))],pairs:slots.every(slot=>slot.querySelectorAll('img').length===2)};
    }""")
    if result['toolbar'] != 4 or result['menu'] < 7:
        raise RuntimeError(f'expected toolbar and menu supplied icons: {result}')
    if not result['loaded'] or not result['dataUris'] or result['inlineChildren'] != 0 or result['inlineSuppliedSvg'] != 0 or result['tags'] != ['IMG'] or not result['pairs']:
