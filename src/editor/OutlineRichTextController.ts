...
import { deleteCodeBlock, findCurrentCodeBlock, removeCodeBlockFormat, replaceCodeBlock, type CodeBlockSnapshot } from './codeBlock';
import { registerYeMindFormats, YEMIND_RICH_TEXT_FORMATS } from './YeMindRichText';
...
export class OutlineRichTextController implements RichTextFormattingTarget {
  ...
  activate(host: HTMLElement, uid: string, request?: OutlineFocusRequest): void {
  ...
  focus(request: OutlineFocusRequest = { placement: 'end' }): void {
    ...
    window.requestAnimationFrame(() => {
      ...
      else if (request.placement === 'select-all') { start = 0; end = length; }
  ...
  getSelectionState(host = this.host): OutlineSelectionState {
    ...
    if (host === this.host && this.quill) {
      ...
      const current = this.quill.getSelection() ?? this.range ?? this.lastRange ?? { index: 0, length: 0 };
      return {
        text,
        length: text.length,
  ...
  detach(commit: boolean): void {
    if (!this.host) return;
    if (commit) this.flush();
    if (this.commitTimer !== null) window.clearTimeout(this.commitTimer);
    this.commitTimer = null;
    const host = this.host;
    const html = this.quill ? normalizeHtml(this.quill.root.innerHTML) : this.originalHtml;
    if (this.quill) {
  ...
  formatText(config: Record<string, unknown>): void {
    ...
    this.quill.formatText(range.index, range.length, config, Quill.sources.USER);
    this.restoreRange(range);
  }
  ...
  private readonly onSelectionChange = (range: any): void => {
    ...
    const format = this.quill.getFormat(range.index, range.length) as Record<string, unknown>;
