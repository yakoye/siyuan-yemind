...
- v0.9.15 writes `yemindClipartGeometryVersion: 2` for newly inserted clipart and repairs only legacy clipart nodes that still carry the old default `72 × 72` size.
- Fixed visual resources remain outside map data and are referenced through stable catalog IDs and plugin-local URLs.

## Release safety

The release archive does not include user data files and can be extracted over the existing plugin folder while SiYuan is closed. The v0.9.15 repair changes only clipart image width, height and a geometry-version marker. Node UIDs, text, hierarchy and unrelated metadata remain unchanged.
