...
The release archive does not include user data files and can be extracted over the existing plugin folder while SiYuan is closed. v0.9.22 does not rewrite node UIDs, text, hierarchy, image data, relation data or unrelated metadata. Checkpoints continue using the existing independent history store.
