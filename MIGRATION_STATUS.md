...
- v0.9.30 adds no persistent map-schema migration. Connector-side resolution, full expansion commands, toolbar pin/icon states, text-to-map menu placement and marker/clipart resource popovers use existing map and settings fields.
- v0.9.31 adds no persistent map-schema migration. It changes only the Theme dropdown DOM and CSS presentation and reads six preview colors from existing preset definitions.
- v0.9.29 adds no persistent map-schema migration. Toolbar pinning adds one settings boolean; editable zoom/title, relation hit geometry, quick-action placement and view-transition cleanup use existing map/view data and runtime UI state.
- v0.9.28 adds no persistent map-schema migration. Compact marker projection, semantic note/comment icons, hover-preview stabilization, click arbitration, clipart resize controls, anchored placement and shared dialog chrome are runtime presentation and interaction changes only.
