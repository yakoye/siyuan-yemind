...
- **Enabled `openRealtimeRenderOnNodeTextEdit`, deleted `liveNodeTextGeometry.ts` and `editingNodeTextSuppression.ts`** (about 260 lines) **and the `.ymz-node-text-editing` CSS rule.** Upstream owns all three behaviours these provided: `Render.onNodeTextEditChange` swaps in a fresh measurement, relayouts the node and repositions the editor; `nodeLayout.js#layout()` paints the edited node's glyphs at `opacity 0` so the Quill overlay is the only text layer; and `TextEdit.show()` takes the glyphs down before the editor layer is even attached. The rc.6/rc.7 reveal timing is upstream's too — `hideEditText` nulls `richText.node` before the commit render, so the same `layout()` restores the glyphs in the frame that drops the editor.
- **Two vendor fixes were needed to make the upstream path correct here**, both in the already-forked `Render.js`:
  - `onNodeTextEditChange` now refreshes `node.nodeDataSnapshot` after a live measurement. `Base#doLayout` re-measures any node whose data changed since the last render, and a YeMind edit session does mutate node data on every keystroke (the edited/pristine marker), so the very next render rebuilt the node's text from the *stale stored text* and discarded the live measurement — the node never visibly resized. Same idiom the width-drag path already uses. Diagnosed by instrumenting the tick in a real browser: the measurement was correct (235px) while the painted `data-width` stayed at the pre-edit 73px.
  - `onNodeTextEditChange` skips the tree relayout when the fresh measurement did not change the node box. Most ticks of a typing burst grow the text within the line it already occupies; relaying out the tree on every 100ms tick regardless was the measured CPU cost of the pre-1.8.0 live-render pipeline. This preserves the one genuinely useful thing the deleted YeMind controller did.
- A fourth contract was quietly wrong in a way that only surfaced once nodes resize live: it sampled the edited node's static text rectangle every frame and asserted it never differs from its final one. Its visibility predicate had the same ancestor-opacity blind spot, so it sampled the intermediate geometry of a node actively resizing under the editor and failed about 40% of runs. With the corrected predicate those frames contribute nothing (the layer is not painted) and the original intent — no misplaced painted geometry — is preserved: 5 of 5 consecutive runs pass.
- Three test contracts encoded the superseded "opaque editor covers a still-valid SVG fallback" design and are rewritten to the inverted one — exactly one layer paints text, and the glyphs are down before the editor attaches. One browser assertion had a real bug of its own: it checked the text element's own computed style, but `opacity` is not inherited and upstream sets it on an ancestor `<g>`, so a hidden layer was reported as painted. It now walks the ancestor chain.
- All behaviour regressions from rc.5 through rc.8 pass unchanged against the upstream implementation: node grows while typing, shrinks while deleting, no doubled text layer, no empty frame on close, 20-character wrap limit, manual width precedence, insertion selection.
- Both fork ledgers updated: `README.YEMIND.md` records the new `Render.js` patch reason, and the removed-coordinators contract test records the two deleted modules so they cannot quietly return.
- Full verification: `tsc --noEmit`, docs check, unit suite (1008 passed / 0 failed), complete Playwright E2E across desktop and mobile projects (144 passed / 0 failed / 0 flaky).
...
- **Fixes a node flashing empty when its editor closes.** rc.6 restored the node's glyphs on the commit's `node_tree_render_end`, on the assumption that the render happens after `hide_text_edit`. Instrumented in Chromium, the real order is the opposite: the commit render completes **synchronously inside `SET_NODE_TEXT`**, so by the time `hide_text_edit` is emitted the render end has already been dispatched, and a listener registered from that handler is waiting for something that already happened. The glyphs then stayed hidden until the 400ms safety timer fired — measured as **25 to 26 consecutive frames painting neither layer**, i.e. a visibly empty node. The reveal is now driven by the state that actually matters: the glyphs come back the moment the opaque host stops covering them, checked directly rather than inferred from an event. Measured after the fix: **0 blank frames**.
- **Re-measures every node once the fonts it was rendered with have loaded** (`src/core/firstPaintGeometry.ts`). Node geometry is measured from the DOM at render time, so a map that paints before its host's webfonts resolve is sized with fallback metrics — wider ones, which makes every node too big — and keeps that size until something else re-measures it, which in practice meant opening and closing an editor on each node one at a time. `changeTheme` is the one render source upstream treats as "geometry is stale" (`Base#checkIsNeedResizeSources`), so it is the source used. Nothing runs on a warm start where the fonts already resolved, under an open editor, or after the map was torn down.
- **Stops a rebuilt editor from blanking a node's text.** A canvas resize or zoom during an open session makes upstream tear the editor down and rebuild it from `cacheEditingText || nodeText`. Quill's empty document is `<p><br></p>`, which is truthy, so a rebuild triggered before the editor mounted its content replaced the node's real text with a blank document — a freshly inserted node losing its `新节点` label. An empty document is now stored as an empty string, so the fallback to the node's own text actually happens.
- Two of the three reports could not be reproduced in a real browser as stated: node geometry was byte-identical before and after a reload (`data-width`/`data-height` and painted shape unchanged), and Tab, Enter and quick-add each opened on `新节点` with the whole label selected. The two changes above address the most probable environment-specific causes — fonts resolving after first paint, and a resize rebuilding the editor mid-open — both of which exist only outside the test harness. If either symptom persists, it needs a fresh repro from the SiYuan runtime.
- Added 8 unit scenarios (reveal timing against host state, font-ready re-measure including its warm-start/open-editor/torn-down guards, empty-document detection) and 4 browser regressions (no blank frame on close; all three insertion entry points opening on a fully selected `新节点`, with the first keystroke replacing it).
- Full verification: `tsc --noEmit`, docs check, unit suite (1016 passed / 0 failed), complete Playwright E2E across desktop and mobile projects (142 passed / 0 failed, no retries).
- Remains an RC until the three reported behaviors are confirmed in the real SiYuan runtime.

## 1.9.9-rc.6 - 2026-08-05

This release candidate makes the open canvas editor the only layer that paints a node's text, and stops a host focus steal from destroying a new node's selection.

- **Fixes doubled, blurred text while typing.** Two layers always painted at once: the SVG glyphs under an opaque editor host. That was harmless while the static text was frozen at the pre-edit content, but rc.5 made the node resize as the user types, so the SVG layer now holds a *different revision of the same string* a few pixels from the Quill layer. Measured in Chromium: across the frames of one typing burst, **28 of 70 (light) and 34 of 72 (dark)** painted a visibly different string in the two layers. The node's glyphs — not its shape, fill or border — are now suppressed for the whole session through one class on the node group and one CSS rule, so exactly one layer ever paints text.
- **Fixes the one-frame flash of text at a node's top-left corner when an editor closes.** The glyphs are restored only on the `node_tree_render_end` of the commit, which is the same event upstream's `hideEditText` uses to drop the opaque host — the two hand over directly, with no frame in between where text can paint at its pre-layout local origin. A 400ms fallback guarantees the glyphs return even if no render follows.
- **Fixes typing into a freshly inserted node appending to `新节点` instead of replacing it.** When the host took DOM focus, `handleHostFocusIn` reclaimed it and restored the selection from `range ?? pasteUseRange ?? quill.getSelection()`. All three are routinely empty — `range` only ever holds a *non-collapsed* selection and is cleared on every keystroke, `pasteUseRange` was skipped for insertions by the `isInserting` short-circuit, and `getSelection()` returns null while focus sits elsewhere — so the fallback "collapse to the end of the text" was the common path, not the exception, and it silently destroyed the full selection an inserted node opens with. Reproduced by simulating the host stealing focus: the selection went from `新节点` to empty, and typing produced `新节点fdsfdsfdfds`. A recorded last-known range now covers the gap, refreshed from both `selection-change` and `text-change` because Quill emits the caret move that follows typing as a suppressed event.
- Implemented as `EditingNodeTextSuppression` (`src/editor/editingNodeTextSuppression.ts`) driven purely by `before_show_text_edit` / `node_text_edit_change` / `hide_text_edit` / `node_tree_render_end`, so the v1.9.0 contract that YeMind never overrides upstream's `showEditText`/`hideEditText`/`removeTextEditEl`/`focus` still holds exactly.
- One report could not be reproduced as a measurable defect: a node's box supposedly growing taller across repeated double-clicks. Open/close cycles — including alternating two nodes and with host focus steals injected — held both nodes' shape, text and declared `data-width`/`data-height` byte-identical over four rounds. The reported symptom is consistent with the doubled-layer artifact above (a second, offset copy of the glyphs reads as text not fitting its frame), so it is expected to disappear with it; if it persists, it needs a fresh repro from the SiYuan runtime.
- Added 8 unit scenarios (glyph suppression lifecycle, reveal timing, fallback, cleanup; focus-restore precedence) and 2 browser regressions, one of which is validated to discriminate: with the suppression disabled it records ~50 frames painting both layers, with it 0.
- Full verification: `tsc --noEmit`, docs check, unit suite (1011 passed / 0 failed), complete Playwright E2E across desktop and mobile projects (138 passed / 0 failed, no retries).
- Remains an RC until the four reported behaviors are confirmed in the real SiYuan runtime.
