# YeMind 启动恢复诊断分析（2026-07-17 13:08）
...
1. 完整替换插件并重启思源后，YeMind Dock 显示“暂无导图”。
...
官方 KMind Zen 公开仓库在本次检查时只提供 README、CHANGELOG 和插件清单，没有可直接复用的编辑器源码。YeMind 因此不复制或反编译其私有内核，而是对齐可观察交互，并调用当前安装的 `simple-mind-map` 原生命令：
