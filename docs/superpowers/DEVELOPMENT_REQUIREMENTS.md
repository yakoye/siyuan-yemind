...
当前稳定开发基线是 **YeMind Zen v0.5.11**。后续开发必须从该版本继续，不得回退到 v0.5.10 或重新引入已经解决的问题。

v0.5.10 与 v0.5.11 的受保护能力包括：

- 诊断生命周期与正式导图库隔离。
- Dock 加载时序和恢复标签页挂载保护。
- 纯大纲模式零尺寸画布保护。
- 待办点击只切换完成状态，删除必须显式触发。
- 拖动父节点虚线只增强上游 Drag 提示，不替换上游结构算法。
...
5. **自动化与手工验收分开。** 自动化测试证明逻辑契约；Windows SiYuan 3.7.2 中的真实鼠标操作证明宿主集成。两者不得互相冒充。
