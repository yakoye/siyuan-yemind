...
当前稳定开发基线由 `package.json`、`plugin.json`、`VERSION`、`web/VERSION` 和源码版本常量的共享版本契约决定，不在本规范中硬编码。后续开发必须从当前一致版本继续，不得回退到旧版或重新引入已经解决的问题。
...
```text
docs/designs/YYYY-MM-DD-HHmm-<version>-<topic>-设计.md
docs/plans/YYYY-MM-DD-HHmm-<version>-<topic>-实施计划.md
docs/releases/vX.Y.Z/YYYY-MM-DD-HHmm-vX.Y.Z-<topic>-验证记录.md
```

真实诊断包驱动的修复可额外形成同样带北京时间分钟前缀的诊断或回归记录。

设计文档描述“为什么和做什么”；计划描述“按哪些文件和测试步骤实施”；验证报告只陈述实际执行过的结果。

长期规范统一放在 `docs/standards/`，使用稳定中文文件名，不添加日期或版本号。设计、计划、回归、构建和版本证据属于历史记录，文件名必须使用 `Asia/Shanghai` 的 `YYYY-MM-DD-HHmm` 前缀。每次开发完成前必须执行 `npm run check:docs`；命名错误、迁移旧路径残留或本地链接断链均阻断提交和发布。
