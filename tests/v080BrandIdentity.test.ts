...
describe('YeMind v0.8.0 brand and engineering identity', () => {
  it('uses YeMind publicly and siyuan-yemind as the engineering package while preserving the legacy plugin id', () => {
    const manifest = JSON.parse(readFileSync(resolve('plugin.json'), 'utf8'));
    const packageJson = JSON.parse(readFileSync(resolve('package.json'), 'utf8'));
    expect(manifest.name).toBe('siyuan-yemind-zen');
