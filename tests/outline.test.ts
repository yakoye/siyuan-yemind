...
describe('outline view model', () => {
  it('flattens the tree with depth, uid and plain text', () => {
    expect(flattenOutline(tree)).toEqual([
      { uid: 'r', text: 'Root', depth: 0, hasChildren: true, expanded: true, isRoot: true },
      { uid: 'a', text: 'Child A', depth: 1, hasChildren: false, expanded: true, isRoot: false },
      { uid: 'b', text: 'Child B', depth: 1, hasChildren: true, expanded: true, isRoot: false },
      { uid: 'g', text: 'Grandchild', depth: 2, hasChildren: false, expanded: true, isRoot: false },
    ]);
  });
});
