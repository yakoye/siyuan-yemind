...
describe('v0.7.x about and diagnostics release contract', () => {
  ...
  it('uses semantic patch version 0.8.3 consistently', () => {
    expect(RELEASE_INFO.version).toBe('0.8.3');
    expect(resolveVersionConsistency('0.8.3')).toEqual({ manifest: '0.8.3', runtime: '0.8.3', build: '0.8.3', consistent: true });
  });
  ...
  it('keeps package, lockfile, manifest, runtime metadata and release docs on one identity', () => {
    ...
    expect(readme).toContain('当前版本：`0.8.3`');
    expect(changelog).toContain('## 0.8.3');
  });
