...
describe('YeMind plugin identity and install layout', () => {
  it('uses siyuan-yemind for the current manifest, package and installed directory', () => {
    ...
    expect(manifest.version).toBe('0.9.17');
    expect(packageJson.version).toBe('0.9.17');
  });
  ...
  it('keeps public documentation on the YeMind identity and SiYuan 3.7.3 baseline', () => {
    const publicDocs = [
      ...
      ...Array.from(new Set([
        ...
        'docs/TEST_COVERAGE_MATRIX_v0.9.17.md', 'docs/verification-v0.9.17.md',
        'docs/PRODUCT_BOUNDARIES_v0.9.17.md',
      ])),
