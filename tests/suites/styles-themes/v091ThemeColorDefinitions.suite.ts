...
describe('v0.9.1 complete theme color definitions', () => {
  it('keeps the original 19-theme JSON as the ordered source of truth', () => {
    const source = JSON.parse(readFileSync(resolve(process.cwd(), 'docs/theme-colors/yemind_theme_colors.json'), 'utf8'));
    ...
    for (const [name, theme] of Object.entries<any>(source.themes)) {
      ...
      expect([theme.background, theme.centerText, theme.centerBackground].every((value) => COLOR.test(value))).toBe(true);
  ...
  it('keeps generated theme fallbacks non-persistent and below local node styles', () => {
    const tree: any = {
      ...
      children: [
        {
          data: { text: 'Branch', fillColor: '#123456', color: '#654321', lineColor: '#ABCDEF' },
    ...
    expect(tree.children[0].data).toMatchObject({
      fillColor: '#123456', color: '#654321', lineColor: '#ABCDEF',
    });
