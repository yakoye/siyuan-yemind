...
describe('v0.7.x about and diagnostics release contract', () => {
  ...
  it('uses the current semantic version consistently', () => {
    expect(RELEASE_INFO.version).toBe('0.9.17');
    expect(resolveVersionConsistency('0.9.17')).toEqual({ manifest: '0.9.17', runtime: '0.9.17', build: '0.9.17', consistent: true });
  });
  ...
  it('keeps package, lockfile, manifest, runtime metadata and release docs on one identity', () => {
    ...
    expect(readme).toContain('当前版本：`0.9.17`');
    expect(changelog).toContain('## 0.9.17');
  });
