...
describe('outer-frame editor integration', () => {
  it('subscribes only to native outer-frame lifecycle events and rerenders through the plugin', () => {
    expect(source).toMatch(/this\.map\.on\(["']outer_frame_active["']/);
    expect(source).toMatch(/this\.map\.on\(["']outer_frame_deactivate["']/);
    expect(source).toMatch(/this\.map\.on\(["']outer_frame_delete["']/);
